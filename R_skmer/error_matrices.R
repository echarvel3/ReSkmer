...
skmer1_mtrx = fread("./skmer1_matrices/skmer1_nematode_skims_dist_mat.txt", header=TRUE)
...
ggplot(data = error_melt, aes(x = variable, y = true_dist, fill = value)) +
  geom_tile() + geom_text(aes(label=round(value, digits = 5))) +
  scale_fill_distiller(palette = "Spectral", direction=.5)
